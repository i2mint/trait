...
ModuleNotFoundIgnore = partial(
    suppress, ModuleNotFoundError, ImportError
...
def mk_callable(call_func):
    ...
    ...     '''NearestNeighbors with callable instances that give you the indices of
    ...     the neighbors without the kerfuffle.'''
    >>>
    >>> knn = CallableKnn().fit(np.arange(1000).reshape(200, 5))
    >>> # say we have a single point we want to get neighbors for:
    ...
    def add_call_method(cls):
        if call_func_str is not None:
            if hasattr(cls, call_func_str):
                ...
                raise ValueError(
                    f"call_func was specified by a string, "
                    f"but was neither the name of the method of the class"
                    f"nor 'single_METHOD_NAME' where METHOD_NAME "
                    f"is a method of the class: {call_func_str}"
                )
        else:
            assert callable(
                call_func
...
def row_euclidean_distance(A, B):
    """Euclidean distance between aligned rows of A. Array of length len(A) (==len(B)).
    ...
    Note: Not to be confused with the matrix of distances of all pairs of rows.
    Here, equivalent to the latter diagnonal (see below).
...
class lazyprop:
    """
    A descriptor implementation of lazyprop (cached property) from David Beazley's
    ...
    >>> t.len
    5
    >>> # But careful when using lazyprop that no one will change the value of a
    >>> #  without deleting the property first
...
def balanced_sample_maker(key_to_tag, max_n_keys_per_tag=7, random=False):
    ...
    ...                                   random=False)
    >>> mk_sample([
    ... 'good/1', 'bad/1', 'good/2', 'good/3', 'good/4', 'bad/2', 'good/5', 'bad/3'
    ... ])
...
class ArithmeDict(dict):
    ...
    When a dict operates with a number, the operation is applied to each value of
    the dict.
    When a dict operates with another dict, the keys are aligned and the operation
    applied to the aligned values.

    The class is meant to be used in situations where pandas.Series would be used to
    operate with (sparse) vectors
    such as word counts, etc.

    Performance:

    In a nutshell, if you use pandas already in your app, then use pandas.Series instead.
    But, if you want weight packages (pandas isn't light), or have small dicts you want
    to operate on, use ArithmeDict.

    Note that both construction and operation are faster on ArithmeDict,
    for smaller sets.
    ...
    On the other hand, memory usage is inconclusive, because I don't know how to
    ...
    there are different merge and reduce options that may or may not make sense
    according to the value type and context.
    For example, should we really keep all keys and use operand defaults to get
    their values, or just drop
    those fields all together?
    Also, if we choose to keep all keys, what should the operand default be.
    Sometimes it might depend on the other operand (example matmul), or need to be
    created (example __concat__,
    ...
    >>>
    >>> # An operation with a number is transferred to the values of the dict
    >>> #  (applied to each).
    ...
    >>>
    >>> # An operation with another dict will align the keys and apply the operation
    >>> #  to the aligned values.
    ...
    >>> # works when the right side is a normal dict:
    >>> assert d1 + {'b': 3, 'c': 4} == {'a': 1, 'b': 5, 'c': 4}
...
def running_mean_gen(
    ...
    Note: When input it is list-like, ut.stats.smooth.sliders version of running_mean
    is 4 times more efficient with
